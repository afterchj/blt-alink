...
public class WebSocketConfig implements WebSocketConfigurer {
    ...
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
        ...
        registry.addHandler(springWebSocketHandler(), "/websocket/pushCode").setAllowedOrigins("*").addInterceptors(springWebSocketHandlerInterceptor());
//        //允许客户端使用SockJS
//        //SockJS 是一个浏览器上运行的 JavaScript 库，如果浏览器不支持 WebSocket，该库可以模拟对 WebSocket 的支持。
        registry.addHandler(springWebSocketHandler(), "/sockjs/pushCode").addInterceptors(springWebSocketHandlerInterceptor()).withSockJS();
    }
